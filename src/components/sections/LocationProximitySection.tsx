...
export default function LocationProximitySection() {
  ...
  const { adjacentData, overlappedData, bothYes, totalBothYes, insightText } = useMemo(() => {
    ...
    const ovlData = SUB_HOLDINGS.map((sh) => {
      ...
      return {
        ...
        null_count: units.filter((u) => u.location_overlapped === null).length,
      };
    });
    ...
    if (shWithBoth.length > 0) {
      const shNames = shWithBoth.map((d) => `${d.label} (${d.count} unit)`).join(" dan ");
  ...
  return (
    <SectionWrapper id="location-proximity" background="white">
      ...
      <p className="section-subtitle">Analisis lokasi operasi terhadap area biodiversitas tinggi</p>

      <div className="mt-6">
        <NarrativeBlock>
          <p>
            Setiap unit operasi dievaluasi berdasarkan dua aspek: (1) apakah lokasi <strong>berdekatan</strong> (adjacent)
            dengan kawasan lindung atau area biodiversitas tinggi, dan (2) apakah wilayah kerja{" "}
            <strong>tumpang tindih</strong> (overlapped) dengan kawasan tersebut. Unit yang memiliki kedua kondisi ini
            memerlukan perhatian khusus dalam pengelolaan dampak lingkungan.
          </p>
        </NarrativeBlock>
      </div>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-2">
        <div className="card">
          <StackedPercentBar data={adjacentData} label="Adjacent to Protected Areas" yesColor="#2E8540" />
        </div>
        <div className="card">
          <StackedPercentBar data={overlappedData} label="Overlapped with Protected Areas" yesColor="#E06919" />
        </div>
      </div>
        ...
        <div className="flex flex-col items-center gap-4 sm:flex-row">
          <div className="text-center sm:text-left">
            ...
            <p className="mt-1 text-sm text-gray-600">
              Unit dengan lokasi berdekatan DAN tumpang tindih dengan kawasan lindung
            </p>
          </div>
